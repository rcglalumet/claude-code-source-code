...
源码提取: Claude Code yoloClassifier.ts / QueryEngine.ts / compact.ts
"""
...
# 以下字符串从 Claude Code 源码 + 旧版防御资产 双向提炼
# ============================================================
...
class ASTSentinel(ast.NodeVisitor):
    ...
    提炼自 Claude Code 安全哲学: Dumb Tools 内部绝对禁止推演逻辑。
    """
...
class ExactGrepSearchTool(MCPToolBase):
    ...
    对齐 Claude Code 的 Exact Search 理念: 宁可精确命中，不要近似幻觉。
    """
...
强制约束 (提炼自 QueryEngine.ts while-true 架构):
...
[V10 升级] 提炼自 Claude Code yoloClassifier.ts 两阶段 XML 判定逻辑:
  - Stage 1 (fast): 低成本规则引擎即时判定，允许则直接放行。
  - Stage 2 (thinking): 仅在 Stage 1 判定为高危时升级，深度链式推理复核。
  这种非对称设计使 99% 正常流量仅经 Stage 1 极速通过，极大降低算力消耗。
...
# Stage 2 升级触发词 (对齐 yoloClassifier.ts XML_S1_SUFFIX 阻断模式)
ESCALATE_PATTERNS = [
...
class YoloVetoClassifier:
    ...
    def classify(self, text: str) -> dict:
        ...
        # --- Stage 2: Thinking path (仅处理 Stage 1 未拦截的可疑输入) ---
        for pattern in self._escalate_re:
            if pattern.search(text):
                return {
                    "vetoed": True,
                    "reason": "Stage 2 深度复核: 检测到升级危险模式",
                    "stage": "thinking"
                }
...
对齐 Claude Code compact.ts: 五级压缩漏斗 + 响应式 413 Hook。
"""
import json
...
CODE_VRAM_LLM_GATEWAY = '''"""全局网络层 - 含 413 Reactive Compact Hook
提炼自 Claude Code compact.ts: 413 响应触发上下文折叠，不中断服务。
"""
...
INFRA_FILE_MAP = {
    ...
    "oracle_gateway/llm_network_gateway.py": CODE_VRAM_LLM_GATEWAY,
}
...
DOMAIN_ROUTER: dict = {
    "1": {
        "domain": "xiangshu",
        "dir": "01_xiangshu",
        "parser": "xiangshu_tensor_parser.py",
        "label": "象数(01)",
    },
    "2": {
        "domain": "bazi",
        "dir": "02_bazi",
        "parser": "bazi_ephemeris_parser.py",
        "label": "八字(02)",
    },
    "3": {
        "domain": "liuyao",
        "dir": "03_liuyao",
        "parser": "liuyao_hexagram_parser.py",
        "label": "六爻(03)",
    },
    "4": {
        "domain": "xuankong",
        "dir": "04_xuankong",
        "parser": "xuankong_grid_parser.py",
        "label": "玄空(04)",
    },
    "5": {
        "domain": "finance",
        "dir": "05_finance",
        "parser": "finance_kline_parser.py",
        "label": "金融(05)",
    },
}

# 5 大标准灌注端口映射 (R/M/T/P/F)
PORT_LABELS = {
...
# ============================================================
# 【终端路由器 V1.2.2】- 5 大标准灌注端口 (R/M/T/P/F)
# 严格按物理坐标映射表路由，绝对禁止泛化拼接
# ============================================================

def interactive_surgical_router():
    pc("\n" + "=" * 62, "blue")
    pc("🔪 [Alumet OS V10 终端手术刀 V1.2.2] 全域基建播种机已激活", "green")
    pc("=" * 62, "blue")
    ...
    while True:
        ...
        pc("┌─────────────────────────────────────────────────────────┐", "blue")
        pc("│  [功能选择]                                              │", "blue")
        pc("│  1-5  门派代码灌注 (5 大标准端口 R/M/T/P/F)            │", "blue")
        ...
        pc("┌─────────────────────────────────────────────────────────┐", "blue")
        pc("│  [5 大标准灌注端口]                                     │", "blue")
        for port, desc in PORT_LABELS.items():
            rendered = desc.format(domain=domain, dir=dir_name, parser=parser)
            pc(f"│  {port}  {rendered:<53}│", "blue")
        pc("└─────────────────────────────────────────────────────────┘", "blue")

        port_choice = input(_c("端口 (R/M/T/P/F): ", "yellow")).strip().upper()

        if port_choice not in PORT_LABELS:
            pc("  ❌ 无效端口，请输入 R/M/T/P/F", "red")
            continue
...
def main():
    ...
    pc("   源码双向提炼: Claude Code TS + V10 旧版防御资产", "green")
    pc("=" * 62, "green")
    time.sleep(0.5)
